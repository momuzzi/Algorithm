...
public class Main {
    ...
    static void bfs(int rx, int ry, int bx, int by) {
        ...
        while (!q.isEmpty()) {
            ...
            if (poll[4] == 10) { // 10번 이하까지의 노드들이 끝났으므로 불가
                return;
            }

            for (int i = 0; i < 4; i++) {
                if (poll[5] == i) continue; // 이전에 같은 방향으로 기울인거는 다시 할 필요 x

                Result result = move(i, poll);

                if (board[result.bx][result.by] == 'O') continue; // 파랑이 빠진거면 불가
    ...
    static Result left(int[] arr) {
        ...
        if (rx == bx) { // 빨간공과 파란공이 같은 행에 위치해 있다면
            // 누가 더 왼쪽에 위치해 있는지 확인
            if (ry < by) { // 빨간 공이 더 왼쪽에 위치해 있다면
                // 빨간공 왼쪽으로 이동
                while (true) {
    ...
    static Result right(int[] arr) {
        ...
        if (rx == bx) { // 빨간 공과 파란 공이 같은 행에 위치해 있다면
            if (ry < by) { // 파란 공이 더 오른쪽에 위치해 있다면
                // 파란공을 먼저 오른쪽으로 이동시킨다
                while (true) {
                    if (board[bx][by + 1] == '#') break;
